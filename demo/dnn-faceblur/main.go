// What it does:
//
// This example uses a deep neural network to perform face blurring.
...
//
// To perform face blurring with the Caffe model:
//
...
//
// To perform face blurring with the Tensorflow model:
//
...
// How to run:
//
// 		go run ./cmd/dnn-faceblur/main.go [videosource] [modelfile] [configfile] ([backend] [device])
//
// +build example
...
func main() {
	if len(os.Args) < 4 {
		fmt.Println("How to run:\ndnn-faceblur [videosource] [modelfile] [configfile] ([backend] [device])")
		return
	}
...
func performBlurring(frame *gocv.Mat, results gocv.Mat) {
	for i := 0; i < results.Total(); i += 7 {
		...
		if confidence > 0.5 {
			...
			right := int(results.GetFloatAt(0, i+5)*float32(frame.Cols())) - 1
			bottom := int(results.GetFloatAt(0, i+6)*float32(frame.Rows())) - 1
			if right > frame.Cols() {
				right = frame.Cols()
			}
